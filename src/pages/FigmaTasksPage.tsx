    ...
    if (showTest) {
        if (testCompleted) {
            ...
            if (score >= 27) {
                ...
                statusAdvice = 'Не переживай — вернись к урокам в разделе "Курсы", пересмотри уроки и попробуй ещё раз. Практика поможет тебе закрепить знания и улучшить результаты.'
                statusColor = 'text-warning'
            } else {
                // 0-45% - Не пройден
                statusEmoji = '❌'
                ...
                statusAdvice = 'Не переживай — вернись к урокам в разделе "Курсы", пересмотри все уроки и попробуй ещё раз. Практика поможет тебе закрепить знания и улучшить результаты.'
                    ...
                    {selectedTask ? (
                        <Card>
                            <div className="flex items-center justify-between mb-4">
                                ...
                                {submittedTasks.has(selectedTask.id) && !JSON.parse(localStorage.getItem('figma_lessons_progress') || '{}')[selectedTask.id] && (
                                    <span className="text-sm font-medium text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
                                        В ожидании
                                    </span>
                                )}
                                {JSON.parse(localStorage.getItem('figma_lessons_progress') || '{}')[selectedTask.id] && (
                            ...
                            <div className="pt-6 border-t border-gray-200">
                                ...
                                <div className="space-y-4">
                                    ...
                                    {/* Статус отправки */}
                                    {selectedTask && submittedTasks.has(selectedTask.id) && !JSON.parse(localStorage.getItem('figma_lessons_progress') || '{}')[selectedTask.id] && (
                                    ...
                                    {/* Статус принятия */}
                                    {selectedTask && JSON.parse(localStorage.getItem('figma_lessons_progress') || '{}')[selectedTask.id] && (
