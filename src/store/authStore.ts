...
interface AuthStore {
    ...
    register: (email: string, password: string, name: string, city: string, role?: 'student' | 'teacher' | 'admin', subscriptionPlan?: 'monthly' | 'yearly' | 'lifetime' | 'family', selectedDirection?: string, teacherApplicationSubject?: string) => Promise<{ success: boolean; message: string }>
...
export const useAuthStore = create<AuthStore>()(
    persist(
        (set, get) => ({
            ...
            register: async (email: string, password: string, name: string, city: string, role: 'student' | 'teacher' | 'admin' = 'student', subscriptionPlan?: 'monthly' | 'yearly' | 'lifetime' | 'family', selectedDirection?: string, teacherApplicationSubject?: string) => {
                try {
                    set({ isLoading: true, error: null })

                    const response = await firebaseAuthAPI.register(email, password, name, city, role, subscriptionPlan, selectedDirection, teacherApplicationSubject)
